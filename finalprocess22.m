...
for k = 1 : 8
    ...
    %         tab_total=[tab_total;tab];
            [acc_w,acc_nw,rec_w,rec_nw,y,tab_w_cl,tab_nw_cl,tab_w_ncl,tab_nw_ncl]=acc_rec_count(tab,MS_TEMP,samplingRateData,[500 0]); %[0,100]);
            if j < 5 
...
[~,sumIdx] = sort(sum(total_order,1),'ascend');
