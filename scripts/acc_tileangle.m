...
xticklabels({'8°','4°','2°','1°','0.5°'})
ylabel('Accuracy')
titlename = ['Total Accuracy'];
title(titlename,'FontSize',12);
figpath='C:\Users\86186\Desktop\fig';
name = [figpath,'/','total_totalacc.png'];
%saveas(gca,name)

r8_w = sum(tab_w_total(:,11)==8);
r8_nw = sum(tab_nw_total(:,11)==8);
r4_w = sum(tab_w_total(:,11)==4);
r4_nw = sum(tab_nw_total(:,11)==4);
r2_w = sum(tab_w_total(:,11)==2);
r2_nw = sum(tab_nw_total(:,11)==2);
r1_w = sum(tab_w_total(:,11)==1);
r1_nw = sum(tab_nw_total(:,11)==1);
rh_w = sum(tab_w_total(:,11)==0.5);
rh_nw = sum(tab_nw_total(:,11)==0.5);

ra = [r8_nw/r8_w,r4_nw/r4_w,r2_nw/r2_w,r1_nw/r1_w,rh_nw/rh_w];
rt = size(tab_nw_total,1)/size(tab_w_total,1);