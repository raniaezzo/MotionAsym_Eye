function [acc_w,acc_nw,rec_w,rec_nw,y,tab_w_cl,tab_nw_cl,tab_w_ncl,tab_nw_ncl]=acc_rec_count(tab,MS_TEMP,samplerate,time)
%lower=(1300-time(1))*samplerate/1000;
%higher=(1800+time(2))*samplerate/1000;

% stimulus time (1800 - 500 --> 1800+300)
% response time (post stimulus - mean of subject RT)
lower=(1800-time(1))*samplerate/1000;
higher=(1800+time(2))*samplerate/1000;

% old
%higher=(1800+(mean(tab(:,14))*100))*samplerate/1000; % * 100 to convert to
%ms % (shouldnt it be 1000 to convert to s? 5-14-2023)
...
rec=tab(:,14)+0.5; % adding 0.5 for stimulus period
rec_w=rec(trial_w);
rec_nw=rec(trial_nw);
