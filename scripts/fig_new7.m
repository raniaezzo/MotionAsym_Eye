...
con = {'Swithch', 'Original'}; 
for k =1 : 6 %subject
    count_edges = zeros(8,16);
    count_acc = zeros(8,16,2); %direction * condtion * accur (16*2) 
    for i = 1 : 2 %condition 
        for j = 1 : 8 % direction
...
%             sample=[sample,samplingRateData];
    ...
    %         ms_name = dir(sprintf('%s.mat', direction{j})).name;
            ...
            for o = 1 : 7 
                count_acc2(:,o+1,:)=count_acc(:,2*o,:)+count_acc(:,2*o+1,:);
            end
            count_acc2(:,1,:)=count_acc(:,1,:)+count_acc(:,16,:);
