...
edf2asc = '/Users/rania/Desktop/RadialBias_pilot1/Analysis/_functions/Eyelink/EDF_Access_API/Example/edf2asc';

for ii = 1:1
    for jj = 1 : 2 % NOT 1:2 (for S07 / S08)
        for kk = 1 : 8
                edf2asc = '/Users/rania/Desktop/RadialBias_pilot1/Analysis/_functions/Eyelink/EDF_Access_API/Example/edf2asc';
                ...
                tmpSave = fileparts(MATpath); % RE
                eventFilename = strcat(subject{ii},'_Switched_',direction{kk}, '_events.mat');
                save(fullfile(tmpSave,eventFilename), 'EVENTS');
                rateFilename = strcat(subject{ii},'_Switched_',direction{kk}, '_rate.mat');
                save(fullfile(tmpSave,rateFilename), 'rate');
                
                saveas(gca, sprintf('%s/MS_events_%s.png',MATpath, direction{kk}))
                figure
                plot(rate)
                saveas(gca, sprintf('%s/MS_rate_%s.png',MATpath, direction{kk}))
                direction{kk}
                
%% RE COMMENTED OUT BELOW
%                 %%
%                 MS_TEMP=getms(MS_fil,samplingRateData);
%                 save(sprintf('%s/%s.mat',MATpath, direction{kk}),'MS_TEMP');
%                 %%
% 
...
%                 saveas(gca, sprintf('%s/MS_dir_amp_%s.png',MATpath, direction{kk}))
%                 figure
%                 polarhistogram(deg2rad(stimpost_ms(:,13)),deg2rad(rho_degs))
%                 saveas(gca, sprintf('%s/MS_dir_component_%s.png',MATpath, direction{kk}))
% 
%                 MS_dirs=stimpost_ms(:,12:13);
%                 save(sprintf('%s/MS_dirs_%s.mat',MATpath, direction{kk}),'MS_dirs');
