...
% calculate accuracy and seperate cardinal and oblique
   ...
   %calculate numbers of different condition
    ...
    %         tab_total=[tab_total;tab]
            ...
            if j < 5
                ...
                tab_nw_total_car = [tab_nw_total_car;tab_nw];
                tab_w_total_car = [tab_w_total_car;tab_w];
            else
                tab_nw = [tab_nw_cl;tab_nw_ncl];
                tab_w = [tab_w_cl;tab_w_ncl];
                tab_nw_total_obl = [tab_nw_total_obl;tab_nw];
                tab_w_total_obl = [tab_w_total_obl;tab_w];
            end
    ...
    for ti = 1:5
        a_nw_car = mean(tab_nw_total_car(tab_nw_total_car(:,11)==tilt(ti),15));
        a_w_car = mean(tab_w_total_car(tab_w_total_car(:,11)==tilt(ti),15));
    ...
    ylabel('Accuracy')
    titlename = [subject{kk},' Accuracy(Cardinal)'];
    ...
    name = [figpath,'/',subject{kk},'_cvoacc.png'];
%    saveas(gca,name)
%     
     acc_with_sub_car(:,kk)=acc_with_car;
     acc_notwith_sub_car(:,kk)=acc_notwith_car;
     acc_with_sub_obl(:,kk)=acc_with_obl;
     acc_notwith_sub_obl(:,kk)=acc_notwith_obl;    
